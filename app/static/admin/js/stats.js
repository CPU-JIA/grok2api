...
const MODEL_COLORS = ['#3b82f6', '#22c55e', '#f97316', '#8b5cf6', '#ec4899', '#06b6d4', '#eab308', '#ef4444', '#14b8a6', '#6366f1'];
...
function updateSummary(summary) {
  if (!summary) return;
  const rateValue = String(summary.success_rate ?? '0').replace('%', '').trim();
  const rateRaw = Number(rateValue);
  const rate = Number.isFinite(rateRaw) ? `${rateRaw.toFixed(1).replace(/\.0$/, '')}%` : '0%';

  if (byId('stat-total-req')) byId('stat-total-req').textContent = formatInt(summary.total);
  if (byId('stat-success-req')) byId('stat-success-req').textContent = formatInt(summary.success);
  if (byId('stat-failed-req')) byId('stat-failed-req').textContent = formatInt(summary.failed);
...
function applyChartActiveState() {
  if (!modelsChart || !Array.isArray(modelItems) || modelItems.length === 0) return;
  const safeIndex = getSafeModelIndex();
  const dataset = modelsChart.data.datasets && modelsChart.data.datasets[0];
  if (!dataset) return;

  dataset.offset = modelItems.map((_, idx) => (idx === safeIndex ? 6 : 0));
  dataset.borderColor = modelItems.map((_, idx) => (idx === safeIndex ? 'rgba(17, 24, 39, 0.9)' : '#ffffff'));
  dataset.borderWidth = modelItems.map((_, idx) => (idx === safeIndex ? 2 : 1));
...
function renderTrendCharts(payload) {
        ...
        datasets: [
          {
            ...
            borderColor: '#10b981',
            backgroundColor: createFillGradient(hourlyCanvas, 'rgba(16, 185, 129, 0.18)', 'rgba(16, 185, 129, 0.02)'),
            fill: true,
            borderWidth: 2.2,
            tension: 0.32,
            pointRadius: 0,
          ...
          },
          {
            ...
            borderColor: '#ef4444',
            backgroundColor: createFillGradient(hourlyCanvas, 'rgba(239, 68, 68, 0.14)', 'rgba(239, 68, 68, 0.02)'),
            fill: true,
            borderWidth: 2.1,
            tension: 0.32,
            pointRadius: 0,
  ...
  if (dailyCanvas) {
    ...
    const dayRate = dayTotal.map((total, index) => (total > 0 ? (daySuccess[index] / total) * 100 : 0));
        ...
        datasets: [
          {
            ...
            data: dayTotal,
            borderColor: '#3b82f6',
            backgroundColor: createFillGradient(dailyCanvas, 'rgba(59, 130, 246, 0.16)', 'rgba(59, 130, 246, 0.02)'),
            fill: true,
            borderWidth: 2.2,
            tension: 0.28,
            pointRadius: 2,
...
function renderStats(payload) {
  const rawModels = (Array.isArray(payload.models) ? payload.models : []).slice(0, 10);
...
function renderLogs(logs, total) {
  ...
  if (summary) {
    summary.textContent = `显示最近 ${total ?? 0} 条请求记录`;
  }
  ...
  logs.forEach((item) => {
    ...
    tdIp.className = 'text-left text-xs text-gray-500';
    ...
    tdDuration.className = 'text-center';
    ...
    tdError.className = 'text-left text-xs text-gray-500';
...
async function refreshAll() {
  ...
  try {
    await Promise.all([loadStats({ silent: true }), loadLogs({ silent: true })]);
  } finally {
